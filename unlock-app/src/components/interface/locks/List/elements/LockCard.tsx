  ...
  return (
    <div className="flex flex-col gap-1">
      <div className="flex items-center gap-1">
        {icon && <Icon icon={icon} size={10} className="text-white" />}
...
export const LocksByNetworkPlaceholder = ({
    ...
    return (
      <div className="flex flex-col gap-1">
        <div className="flex gap-2">
          <div className="w-4 h-3 animate-pulse bg-gray-600"></div>
          <div className="w-10 h-3 animate-pulse bg-gray-600"></div>
        </div>
        <div className="w-20 h-5 animate-pulse bg-gray-600"></div>
      </div>
    )
  }
  ...
  const LockCardPlaceHolder = () => {
    return (
      <div className="flex items-center px-12 py-4 bg-gray-800 md:h-24 rounded-2xl">
        <div className="grid items-center justify-between w-full grid-cols-1 gap-4 md:grid-cols-7">
          <div className="flex gap-7 md:gap-3 md:col-span-3">
            <div className="rounded-full bg-gray-600 animate-pulse h-14 w-14"></div>
            <div className="flex flex-col gap-2">
              <div className="h-6 w-52 animate-pulse bg-gray-600"></div>
              <div className="flex items-center gap-3">
                <div className="w-32 h-4 animate-pulse bg-gray-600"></div>
                <div className="w-4 h-4 animate-pulse bg-gray-600"></div>
                <div className="w-4 h-4 animate-pulse bg-gray-600"></div>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-between gap-2 md:col-span-1 md:ml-auto">
            <div className="w-40 h-6 animate-pulse bg-gray-600 md:hidden"></div>
            <div className="w-6 h-6 animate-pulse bg-gray-600"></div>
          </div>
        </div>
      </div>
    )
...
export const LockCard = ({ lock, network }: LockCardProps) => {
  ...
  return (
    <>
      <div className="px-12 py-4 bg-gray-800 shadow-lg rounded-2xl">
        <div className="grid items-center justify-between grid-cols-1 gap-7 md:gap-4 md:grid-cols-7">
          <div className="flex gap-3 md:col-span-3">
            <LockIcon lock={lock} />
            <div className="flex flex-col gap-2">
              <span className="text-2xl font-bold text-white">{lock.name}</span>
              <div className="flex items-center gap-3">
                <span className="text-white">{addressMinify(lockAddress)}</span>
                <Button variant="borderless" onClick={setCopied}>
                  <CopyIcon size={20} className="text-white" />
                </Button>
                <a href={explorerUrl} target="_blank" rel="noreferrer">
                  <Button variant="borderless">
                    <ExternalLinkIcon size={20} className="text-gray-500" />
                  </Button>
                </a>
              </div>
            </div>
          ...
          <div className="grid items-center grid-cols-4 gap-3 md:col-span-3 md:gap-14">
            ...
            <Detail
              ...
              isLoading={isLoading}
            />
            <Detail label="Ticket Duration" value={duration} icon={TimeIcon} />
            <Detail
              label="Ticket Sold"
